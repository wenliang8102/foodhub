...
阶段必须分别编写中文设计和中文实施计划，在同一企业级功能分支上形成独立提交
和测试检查点；六个阶段全部完成后统一提交一个 Pull Request。
...
9. 每个阶段都有独立设计、实施计划、测试和聚焦提交，最终统一创建一个 Pull Request。
...
- Pull Request #1 已完成阶段 0 动态能力交付；
- 阶段 1 至阶段 6 全部在同一个 `codex/social-enterprise` 长期功能分支上实施；
- 每个阶段只形成聚焦的本地提交和测试检查点，不单独创建 Pull Request；
- 一个阶段未通过完整 Social 回归时，不开始下一阶段的业务实现；
- 新增响应字段必须提供兼容默认值，不删除或重命名现有字段；
- 阶段 1 会把现有权限不足和资源不存在响应从统一 HTTP 400 修正为 403 和 404；
  业务错误码保持不变，该有意的状态码兼容影响必须在最终 Pull Request 中说明；
- 六个阶段全部完成后，执行完整测试、边界检查和文档核对，再统一创建一个
  Pull Request；
- 最终 Pull Request 必须列出全部接口、迁移、身份和角色、Redis Key、兼容影响、
  测试结果、故障降级方式以及跨服务假设；
- 仓库负责人合并前执行根构建和 Gateway 冒烟检查；
- 如果阶段需要修改公共模块、Gateway 或基础设施，必须从当前范围移出并先与仓库
  负责人协调。
